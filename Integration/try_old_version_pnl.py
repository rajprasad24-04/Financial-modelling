...
def pnl_table_to_long(df: pd.DataFrame) -> pd.DataFrame:
    """
    Converts a Profit and Loss table to long format:
...
# =================================================
# STEP 2: Collect ALL pnl tables + preserve order
# =================================================

def collect_pnl_long_with_order(all_results: dict):
    """
    Returns:
...
def horizontal_stack_pnl(all_results: dict) -> pd.DataFrame:
    ...
    long_df, order_map = collect_pnl_long_with_order(all_results)
    ...
    # # 🔒 Enforce strict first-seen global order
    # ordered_particulars = sorted(order_map, key=lambda x: order_map[x])
    # merged = merged.set_index("Particulars").reindex(ordered_particulars).reset_index()
...
if __name__ == "__main__":
    all_results = parse_all_pdfs(tickers=["RELIANCE"])
    ...
    final_pnl.to_csv("output/TCS_merged_pnl_sheet.csv", index=False)

    print("\n✅ Exported: output/TCS_merged_pnl_sheet.csv")